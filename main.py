...
def main():
    ...
    logging.info(f"処理対象ページ数: {len(pages)}")
    ...
    try:
        ...
        for i, page_text in enumerate(pages, 1):
            ...
            image_prompt = image_generator.generate_default_image_prompt(page_text)
            logging.info(f"画像生成中 ({i}/{len(pages)}): {image_prompt}")
            
            if not image_generator.generate_image(image_prompt, str(image_path)):
